"""
Combined Background SHAP + Path-Dependent SHAP experiment.

Compares 3 implementations across Fraud Detection and KDD Intrusion datasets:
  ...
  - _WoodelfHDApproach           (memory_crash for D >= 18 — baseline reference)
...
--------
  ...
  Path-Dependent SHAP Interactions — Fraud XGBoost n=10k:
    XGBoost D = 9, 12, 18

Run from the project root:
    python -m benchmarks.woodelf_hd_vs_woodelf_sparse
"""
...
_HD_CRASH_DEPTH = 17  # D > 17 (i.e. D >= 18) crashes for WoodelfHD


class _WoodelfHDApproach(WoodelfHDApproach):
    """WoodelfHD with memory_crash=True for D >= 18 (avoids OOM / extreme slowness)."""
...
XGB_DEPTHS_SMALL  = [9, 12, 18]                # for m=10k and PD interactions
...
def build_missions(cache_root: Path = CACHE_ROOT) -> list[Mission]:
    ...
    # ------------------------------------------------------------------
    # Path-Dependent SHAP Interactions — Fraud XGBoost
    # ------------------------------------------------------------------
    int_task = Task(TaskType.PATH_DEPENDENT_INTERACTIONS, _APPROACHES, n_repeats=1, cache_root=cache_root)
    missions.append(Mission(MissionConfig(
        name="fraud PD SHAP Interactions n=10k (XGBoost T=10)",
        dataset=fraud,
        model_wrappers=fraud_xgb_small,
        tasks=[int_task],
        n_values=[10_000],
        m_values=[0],
        cache_root=cache_root,
...
def build_experiment() -> Experiment:
    return Experiment(
        name="woodelf_hd_vs_woodelf_sparse",
